...
import org.multibit.utils.LeafUtils;
                ...
                if (exchangeSymbols != null) {
                    ...
                    if (!Boolean.FALSE.toString().equals(controller.getModel().getUserPreference(ExchangeModel.TICKER_SHOW))
                        ...
                        if (getItFromTheServer) {
                            ...
                            log.debug("Getting LEAF conversion");
                            float leafRate = LeafUtils.requestLeafBtcConversion();
                            if (leafRate == 0f)
                            {
                                log.debug("Problem getting LEAF conversion");
                                return;
                            }

                            this.exchangeController.getModel().getExchangeData(shortExchangeName).setLastPrice(currency, last.multipliedBy(leafRate));
                            this.exchangeController.getModel().getExchangeData(shortExchangeName).setLastBid(currency, bid.multipliedBy(leafRate));
                            this.exchangeController.getModel().getExchangeData(shortExchangeName).setLastAsk(currency, ask.multipliedBy(leafRate));
                            ...
                            if (isFirstExchange) {
                                ...
                                CurrencyConverter.INSTANCE.setRate(last.getAmount().multiply(BigDecimal.valueOf(leafRate)));
                            }
                        }
                    }
                }
